...
class APIManager {
  ...
  Future<dynamic> getAPICall({required String url, bool isLoaderShow = true, var authToken, bool isToken = false}) async {
    var responseJson;
  ...
  Future<dynamic> postAPICall({required String url, required Map params, bool isToken = false, bool isLoaderShow = true}) async {
    var responseJson;
    print("[Calling API] => $url");
  ...
  Future<dynamic> patchAPICall({required String url, required Map params, bool isLoaderShow = true, bool isToken = false}) async {
    print("Calling API: $url");
    print("Calling parameters: $params");
    ...
    try {
      ...
      final response = await http.patch(Uri.parse(url), headers: headers, body: json.encode(params)).timeout(Duration(seconds: 15)).onError(
        (error, stackTrace) {
          errorSnackBar(message: 'No Internet Connection');
          throw FetchDataException('No Internet Connection');
        },
